...
What separates them is **share of PR churn hidden**. The guard is cumulative per PR, so the numbers that matter are per PR, not per group — a distinction that cost a review round: obs#1328 hides **15.6%**, obs#1335 **40.3%**, layout#523 **43.6%**, versus dbfle#14's **58.8%**. Guard = similar folding may hide at most **50%** of a PR's churn, cumulative across groups. Real headroom is therefore ~6 points, not the ~17 the per-group maximum (33.4%) suggests. Deletion folds are exempt by design — their claim is honest about unread code, and #16595 legitimately folds 96.6%.

A second bound covers a hole the first cannot reach: a similar fold's members are a subset of what deletion folding left behind, so once deletions hide half a diff, `hidden < survivors ≤ total/2` makes the whole-PR cap **mathematically unsatisfiable** — a group could hide everything still readable while the cap stays silent. `maxSurvivorSharePct = 90` bounds hiding against surviving churn. Deliberately loose and uncalibrated: it is non-binding on every known case (nothing deleted there, so both bases are equal) and exists to close the dead zone, not to tune behavior. Evidence that would let it be tightened: split `runs.jsonl`'s `folded_lines_pct` into deletion and similar shares, then look for PRs where a big deletion fold co-occurs with a survivor group. **Not done — independent of this branch.**

`TestFoldSimilarShareGuard` pins each PR whole (groups together, asserting fold counts and hidden-line totals) plus the inclusive 50% boundary; `TestFoldSimilarSurvivorBackstop` pins the delete-heavy case. Mutation-verified: thresholds 34, 40 and 58, the `>`/`>=` boundary, guard removal, cumulative accounting, and backstop removal are all caught. The earlier per-group fixture let 34–43 pass while silently breaking two of the seven PRs it named.

Verified after the change: dbfle#14 folds nothing and regroups into all-claim cohorts with tests paired to their subject (was 5 incl. the false mechanical, and grouping got *faster*: 119s → 58s). Cohort COUNT is not stable — later runs gave 4 and 5; what reproduces is the absence of the false mechanical cohort, not a particular grouping. all three goldens pass, and their fold plans recomputed offline are byte-identical (0 / 0 / one deletion fold, 215 members, 14,877 lines) — the fold plan is deterministic from the stored diffs, so no LLM re-run is needed to prove it.

Known limits: 7 positive vs 1 negative sample; the 43.6%→58.8% gap is real but single-sourced. Failure mode is under-folding (bigger prompt, greedy budget absorbs it) — the direction `fold.go` says to err in. Unrelated observation from the same runs: on one apigw-18 pass the model hunk-split a test file and left 4 hunks unclaimed; repair swept them into the fallback bucket, partition intact — the safety net doing real work.
...
**Residue the reword introduced — unresolved, first thing to try next.** The old conjunction gave folded and mechanical-shape cohorts an implicit escape from `misc` (folded files *do* belong with their fold). The new wording explicitly cancels that ("NOT … that no other cohort would take it"), so an undeclared SIMILAR group now satisfies two rules demanding different types, and mechanical churn is usually undeclared too. Run 2's `[misc]`-instead-of-`[mechanical]` on apigw's pulsar cleanup is consistent with exactly this ambiguity. The fix is probably one precedence clause (fold-line and mechanical-shape types win over `misc`) — but it belongs with the N-run distribution experiment, not another single-sample reword.
