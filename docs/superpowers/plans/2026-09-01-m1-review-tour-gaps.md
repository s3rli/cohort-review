...
Verified after the change: dbfle#14 folds nothing and regroups into 4 clean claim cohorts with tests paired to their subject (was 5 incl. the false mechanical, and grouping got *faster*: 119s → 58s); all three goldens pass, and their fold plans recomputed offline are byte-identical (0 / 0 / one deletion fold, 215 members, 14,877 lines) — the fold plan is deterministic from the stored diffs, so no LLM re-run is needed to prove it.
