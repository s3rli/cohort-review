...
- **The page always renders**: grouping failures (LLM error, bad JSON) must degrade — retry once, then a single "All changes" cohort. Never exit or serve an error page because of the LLM. Repair rules in `cohorts.go` guarantee every changed file lands in exactly one cohort; keep that invariant.
- Server binds 127.0.0.1 only; diffs may be proprietary.

## Testing

`go test ./...` — pure parts only. The LLM is injected as `groupFunc` (canned outputs in tests); never call claude or the network in tests. E2E needs real creds (`BITBUCKET_EMAIL`/`BITBUCKET_API_TOKEN`) + a logged-in `claude` CLI: run against a real PR URL and check every file appears in exactly one cohort.
